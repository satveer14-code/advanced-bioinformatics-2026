...
# Variant files
RAW_VCF="$VARIANT_DIR/NGS0001_raw.vcf"
RAW_VCF_GZ="$VARIANT_DIR/NGS0001_raw.vcf.gz"
RAW_VCF_TBI="$RAW_VCF_GZ.tbi"
FILTERED_VCF="$VARIANT_DIR/NGS0001_filtered.vcf"
...
#######################################
# Step 10: Call variants with Freebayes
#######################################

print_section "Step 10: Variant calling with Freebayes"

if [[ -f "$RAW_VCF_GZ" && -f "$RAW_VCF_TBI" ]]; then
    echo "Compressed/indexed raw VCF already exists. Skipping raw VCF generation."
elif [[ -f "$RAW_VCF" ]]; then
    echo "Raw VCF already exists. Compression/indexing will be handled in Step 11."
else
    freebayes \
      -f "$REF_FASTA" \
      -t "$TARGET_BED" \
...
print_section "Step 11: Compress and index raw VCF"

if [[ -f "$RAW_VCF_GZ" && -f "$RAW_VCF_TBI" ]]; then
    echo "Compressed/indexed raw VCF already exists. Skipping Step 11."
else
    bgzip -f "$RAW_VCF"
    tabix -p vcf "$RAW_VCF_GZ"
    echo "Raw VCF compression and indexing completed."
fi
